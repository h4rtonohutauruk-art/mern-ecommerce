...
export const getAllProducts = async (req, res) => {
  try {
    const products = await Product.find(); //find all produtcs
    res.json({ products });
  } catch (error) {
...
export const getRecomendedProducts = async (req, res) => {
  try {
    console.log("ini dari backend recommendations : ", res);
...
export const getProdutcsByCategory = async (req, res) => {
  const { category } = req.params;
  // console.log("ini req params dari backend ", req.params);
  try {
    const product = await Product.find({ category });
    res.json(product);
    // console.log("ini product dari backend ", product);
    // console.log("ini req params dari backend ", { category });
  } catch (error) {
    console.log("Error in getProdutcsByCategory controller", error.message);
    res.status(500).json({ message: "Server error", error: error.message });
  }
};
